...
\begin{tikzpicture}
  ...
  \draw[->, thick] (v)--(g);
\end{tikzpicture}
\captionof{figure}{\footnotesize Subgroup diagram of the Galois group}
\clearpage
...
\begin{example}
  ...
  Since the intersection of \(\mathbb{Q}(\sqrt{2})\) and \(\mathbb{Q}(\sqrt{3})\) is trival the Galois group of \(f\) is \(G\) itself which is \textit{Klein 4-group}. %AlgorithmsofGaloiscitetation
